...
add_xy <- function(df){
  pts <- sp::SpatialPoints(coords = df[, c("longitude", "latitude")], proj4string = CRSll) %>% 
...
add_latlon <- function(df){
  pts <- sp::SpatialPoints(coords = df[, c("x", "y")], proj4string = CRSutm) %>% 
...
check_points <- function(obj, id){
  
  plot(obj[[id]]$buffer, col = "lightgrey", axes = TRUE, xlab = "Easting (m)", ylab = "Northing (m)")
  plot(obj[[id]]$segment, add = TRUE)
  if(!is.null(unlist(obj[[id]]$presence))){
    purrr::walk(.x = 1:length(obj[[id]]$presence), 
                .f = ~plot(obj[[id]]$presence[.x,], add = TRUE, pch = 16, col = .x + 1))
    purrr::walk(.x = 1:length(obj[[id]]$presence), 
                .f = ~plot(obj[[id]]$exclusion[.x,], add = TRUE, pch = 16, border = .x + 1))
  }
  points(obj[[id]]$absence, pch = 16, cex = 0.85, col = "black")
}
...
get_wkclimatology <- function(remote.dataset,
  ...
  satellite.dat <- function(data.info, lon.extent, lat.extent, timespan, field.name){
    ...
    rerddap::griddap(datasetx = data.info,
...
extract_climg <- function(dat,
  ...
  # if("x" %in% names(dat)) names.col <- c("x", "y") else names.col <- c("longitude", "latitude")
  purrr::map_dbl(.x = 1:nrow(dat), 
                 .f = ~{
                   pb$tick()$print()
                     # raster::projectRaster(from = ., crs = CRSll) %>% 
                     raster::extract(x = climg[names(climg)==as.character(dat[.x, var.name])][[1]], y = dat[.x, c("x", "y")])
                 }, .pb = pb)
...
calc_fcpi <- function(chla.climg, log.10 = FALSE){
  ...
  png("fig/Figure-fcpi.png", res = 450, height = 3500, width = 2500)
  par(mfrow = c(2,1))
  plot(value ~ week, data = chla.2, ylab = "Standardised Chl-a", xlab = "Week")
  lines(chla.2$week, chla.preds$fit, col = "orange", lwd = 1)
  lines(chla.2$week, chla.preds$fit+chla.preds$se.fit, col = "darkgrey", lty = 2)
  lines(chla.2$week, chla.preds$fit-chla.preds$se.fit, col = "darkgrey", lty = 2)
  points(value ~ week, data = chla.2, ylab = "Standardised Chl-a", xlab = "Week")
  ...
  chla.4 <- raster::resample(chla.4, depth)
  
  
  # chla.4 <- raster(chla.4)
...
plot_correlogram <- function(model, data, maxD = 15000, nboot = 500, byrow = FALSE, calc.data=FALSE){
...
list.model.sets <- function(response.var, 
  ...
  use.dat <- dat %>% dplyr::rename(response = response.var)
  ...
  if(reff) gterms <- paste0(gterms, "+ s(weekf, bs = 're')")
  
  if(use.weights){
    
    null.model <- mgcv::gam(formula = as.formula(paste0("response ~ ", gterms, collapse = "+")), 
                            data = use.dat, 
                            weights = gamweights, 
                            REML = TRUE, 
  ...
  if(reff) model.set$mod.formula <- 
    purrr::map(.x = model.set$mod.formula,
               .f = ~as.formula(paste0("~", as.character(.x)[[2]], " + s(weekf, bs = \"re\")")))
...
make_map <- function(input.raster, 
  ...
  ggplot() + 
    geom_tile(data = input.raster, aes(x = lon, y = lat, fill = z)) +
    {if(show.canyons) geom_path(data = canyons.plot, aes(long, lat, group=group))}+
    geom_sf(data = sf::st_as_sf(timor_utm)) +
    coord_sf(xlim = raster::extent(study.area_utm)[1:2], ylim = raster::extent(study.area_utm)[3:4]) +
...
quickplot <- function(input.raster, breaks = NULL, sightings = FALSE, save = FALSE){

  n <- deparse(substitute(input.raster))
  if(save) pdf(paste0(n, ".pdf"), width = 8, height = 6)
  plot(terra::project(terra::rast(input.raster), CRSll), 
       breakby = "cases",
       breaks = breaks,
       pax = list(cex.axis = 1.5),
       plg = list(cex = 1.5),
       col = pals::viridis(100))

  plot(timor, col = "grey", add = TRUE)
  plot(study.area, add = TRUE)
  if(sightings) bw.df |> dplyr::filter(presence == 1) |> dplyr::select(longitude, latitude) |> as.matrix() |> points(pch = 16)
  box()
  if(save) dev.off()
}
...
fit_and_plot <- function(param, k = 4, plot.title = NULL){
  f <- as.formula(paste0("presence ~ s(", param, ", k = ",k,", bs = \"tp\")"))
  mod0 <- mgcv::gam(f,
                    data = bw.df,
                    weights = bw.df$weights,
                    family = binomial(link = "logit"),
                    method = "REML")
  plot(mod0, shade = TRUE, scale = 0, xlab = plot.title); abline (h = 0, lty = 2)
  abline(v = quantile(sort(bw.df[, param]), 0.025), col = "orange", lty = 2)
  abline(v = quantile(sort(bw.df[, param]), 0.975), col = "orange", lty = 2)
  summary(mod0)
}
